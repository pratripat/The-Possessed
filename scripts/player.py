...
class Player(Entity):
    ...
    def __init__(self, game, animations, position, data=None):
        if data != None:
            self.DATA = data
    ...
    def render(self):
        ...
        if self.attacking:
            self.weapon.render(self.game.screen, self.game.camera.scroll)

    def run(self):
        if self.movement_timer <= 0:
            self.movement(self.game.dt, self.game.particles)

        self.update(self.game.dt)
        self.skill_manager.update_skills(self.game.dt)

        if self.invincible_timer > 0:
            self.invincible_timer -= 1

        if self.attacking:
            self.weapon.run(self.game.dt)
    ...
    def movement(self, dt, particles):
        ...
        if self.directions['left'] and not self.directions['right'] and (not self.attacking or self.airtimer > 3):
            self.velocity[0] -= 1
            self.velocity[0] = max(-self.speed, self.velocity[0])
            self.flip(True)
            animation_state = 'run'
        if self.directions['right'] and not self.directions['left'] and (not self.attacking or self.airtimer > 3):
            self.velocity[0] += 1
            self.velocity[0] = min(self.velocity[0], self.speed)
            self.flip(False)
            animation_state = 'run'
        if not self.directions['left'] and not self.directions['right']:
        ...
        if self.directions['down']:
            self.velocity[1] += 1
        #Jump
        elif self.directions['up'] and not self.attacking:
            if self.airtimer < 6:
                self.velocity[1] -= 2
                self.airtimer += 1
    ...
    def update_animations(self, dt, animation_state):
        ...
        if self.current_animation_id == 'player_fall' and int(self.current_animation.frame) != self.current_animation.animation_data.duration():
            return
    ...
    def attacks(self, enemies):
        if self.thorns:
            self.game.projectiles.append(Projectile(self, enemies, self.position, self.image.get_size(), self.attack_damage, 2))

        if self.attack_timer > 0:
            self.attack_timer -= 1

        if self.attacking and self.attack_timer == 0:
            self.weapon.attack(enemies, self.game.projectiles)
